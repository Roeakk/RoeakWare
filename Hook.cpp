...
LRESULT __stdcall WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	if (GUI::getIsInit())
	{
		if (msg == WM_KEYDOWN && wParam == 0x52) {
			GUI::aimbotEnabled = !GUI::aimbotEnabled;
		}
		if (msg == WM_KEYDOWN && wParam == 0x47) {
			GUI::triggerbotEnabled = !GUI::triggerbotEnabled;
		}
		if (msg == WM_KEYDOWN && wParam == 0x46) {
			GUI::leftclickEnabled = !GUI::leftclickEnabled;
		}
		if (msg == WM_KEYDOWN && wParam == 0x43) {
			GUI::chamsEnabled= !GUI::chamsEnabled;
		}

		if (msg == WM_KEYDOWN && wParam == 0x5A) {
			GUI::esp2dEnabled = !GUI::esp2dEnabled;
		}
